...
export default function TransactionsPage() {
    ...
    const handleConfirmReceipt = async (transactionId) => {
        if (!confirm("Are you sure you want to confirm receipt? This action releases the funds to the seller and cannot be undone.")) {
            return; 
        }
        ...
        try {
            ...
            if (res.ok) {
                alert("SUCCESS! Receipt confirmed and funds released.");
                // CRITICAL FIX: After successful completion, force a full data re-fetch
                fetchTransactions('authenticated'); 
            } else {
                const data = await res.json();
                alert(`Error confirming receipt: ${data.message}`);
            }
        } catch (error) {
            alert("A network error occurred while confirming receipt.");
        }
    };
    
    // Function to handle the Seller marking the item as DELIVERED
    const handleMarkAsDelivered = async (transactionId) => {
        if (!confirm("Confirm material has been shipped/delivered? This updates the Buyer's view.")) {
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                alert("SUCCESS! Buyer has been notified the material is delivered and can now confirm receipt.");
                // CRITICAL FIX: After successful status change, force a full data re-fetch
                fetchTransactions('authenticated'); 
            } else {
                 const data = await res.json();
                alert(`Error marking as delivered: ${data.message}`);
            }
        } catch (error) {
