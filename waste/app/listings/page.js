...
import Head from 'next/head'; // Head is still often used for metadata
import Link from 'next/link';
import { useRouter } from 'next/navigation'; // *** MIGRATION FIX: New router location ***

export default function ListingsPage() {
  const { data: session, status } = useSession(); 
  const router = useRouter(); // Initialize App Router navigation
  ...
  const renderAuthButton = () => {
    ...
    return (
      // FIX: Use modern Link component pattern (no passHref needed for span/text children)
      <Link href="/login"> 
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {/* Header/Nav Bar Simulation (Uses App Router structure) */}
      <div className="flex justify-between items-center mb-8 border-b pb-4">
        <Link href="/">
          <h1 className="text-2xl font-extrabold text-gray-800 cursor-pointer">WasteWise Exchange</h1>
        </Link>
