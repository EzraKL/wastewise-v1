'use client'; // CRITICAL: Required for client-side hooks (useState, useRouter, useSession)

import { useRouter, useSearchParams } from 'next/navigation'; // *** MIGRATION FIX: New router location ***
...
export default function PaymentPage() { 
    ...
    // CRITICAL: Use the App Router's way to access query params
    const searchParams = useSearchParams();
    const transactionId = searchParams.get('transactionId'); 
    
    const { status } = useSession();
    
    // State management for button and messages
    ...
    if (status === 'unauthenticated') {
        router.replace('/login'); // Use replace to clear history on redirect
        return null;
    }
    ...
    // --- Core Logic: Handle Payment Simulation API Call ---
    const handleSimulatePayment = async () => {
        setSubmitStatus({ message: 'Contacting payment gateway...', loading: true, success: false });
        
        try {
            // CRITICAL CALL: Submitting the transaction ID to the secured API
            ...
            if (!res.ok) {
                // If payment API returns 400 or 500
                setSubmitStatus({ 
